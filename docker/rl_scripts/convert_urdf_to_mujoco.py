#!/usr/bin/env python3
"""
Convert DOFBOT URDF to MuJoCo XML format
Uses MuJoCo's built-in URDF loader
"""
...
def convert_urdf_to_mujoco(urdf_path, output_path):
    """
    Convert URDF to MuJoCo XML

    Note: MuJoCo can load URDF files directly, but we'll create
    a MuJoCo-specific XML for better control over simulation parameters
    """

    # MuJoCo XML template with URDF include
    mujoco_xml = f"""<mujoco model="dofbot">
...
"""
    ...
    print(f"✓ Created MuJoCo XML: {output_path}")
    print(f"  Model: DOFBOT 5-DOF robot arm")
    print(f"  Joints: 5 revolute joints with position control")
    print(f"  Meshes: Referenced from {os.path.dirname(urdf_path)}/meshes/")


def main():
    # Paths
    urdf_path = "/root/dofbot_ws/src/dofbot_moveit/urdf/dofbot.urdf"
    output_path = "/root/mujoco_models/dofbot.xml"

    print("Converting DOFBOT URDF to MuJoCo XML...")
    print(f"Input:  {urdf_path}")
    print(f"Output: {output_path}")
    print()

    convert_urdf_to_mujoco(urdf_path, output_path)

    print()
    print("Conversion complete!")
    print()
    print("To test the model:")
    print("  python3 -m mujoco.viewer /root/mujoco_models/dofbot.xml")
    print()
    print("To use with ROS bridge:")
    print("  roslaunch dofbot_config demo.launch")
    print("  python3 /root/rl_scripts/ros_mujoco_bridge.py /root/mujoco_models/dofbot.xml")
